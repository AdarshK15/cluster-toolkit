...
| <a name="input_enable_expedited_requeue"></a> [enable\_expedited\_requeue](#input\_enable\_expedited\_requeue) | Enables Expedited Requeue, which automatically requeues eligible jobs and grants them the highest priority upon node failure. (Usage: sbatch --requeue=expedite) | `bool` | `false` | no |
...
| <a name="input_slurm_bucket_kms_key"></a> [slurm\_bucket\_kms\_key](#input\_slurm\_bucket\_kms\_key) | Customer-managed encryption key self-link to use for the Slurm configuration bucket. | `string` | `null` | no |
...
| <a name="input_subnetwork_stack_type"></a> [subnetwork\_stack\_type](#input\_subnetwork\_stack\_type) | Stack type of the subnet to deploy to. | `string` | `"IPV4_ONLY"` | no |
