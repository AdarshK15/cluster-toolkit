...
def get_instance_template_copy(nodeset: NSDict, lkp: util.Lookup) -> str:
    ...
    properties = dict(source_template.get("properties", {}))
    properties["labels"] = {
...
def get_mig_for_node(node: str, lkp: util.Lookup) -> tuple[Optional[str], List[str]]:
    ...
    for mig_item in mig_items:
        desc = mig_item.get("description", "")
        peers = desc.split(":", 1)[1].split(",") if desc.startswith("slurm_nodes:") else []
...
def _resume_single_tpu_node(
    ...
    if existing_mig:
        log.info("TPU node %s already managed by MIG %s. Skipping.", first_node, existing_mig)
        return
    model = chunk[0]
    nodeset = lkp.node_nodeset(model)
    region = lkp.node_region(model)
    ...
    mig_req_body = dict(
        ...
        description=f"slurm_nodes:{','.join(chunk)}",
    ...
    res = util.wait_for_operation(op)
    if "error" in res:
        raise RuntimeError(f"TPU MIG creation operation failed: {res}")
    ...
    res = util.wait_for_operation(op)
    if "error" in res:
        raise RuntimeError(f"TPU MIG resize operation failed: {res}")
